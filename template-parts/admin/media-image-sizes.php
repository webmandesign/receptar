...
$default_image_size_names = array(
	...
	'medium_large' => esc_html_x( 'Medium large size', 'WordPress predefined image size name.', 'receptar' ),
...
<div class="recommended-image-sizes">
	...
	<h3><?php esc_html_e( 'Recommended image sizes', 'receptar' ); ?></h3>

	<p>
		<?php esc_html_e( 'For the optimal theme display, please, set image sizes recommended in table below.', 'receptar' ); ?>
		<?php esc_html_e( 'If you already have images uploaded to your website you need to resize them after changing the sizes here.', 'receptar' ); ?>
		<a href="<?php echo esc_url( $resize_url ); ?>"><?php esc_html_e( 'Resize images using plugin &rarr;', 'receptar' ); ?></a>
	</p>
	...
	<table>
		...
		<thead>
			<tr>
				<th><?php esc_html_e( 'Size name', 'receptar' ); ?></th>
				<th><?php esc_html_e( 'Size ID', 'receptar' ); ?></th>
				<th><?php esc_html_e( 'Size parameters', 'receptar' ); ?></th>
				<th><?php esc_html_e( 'Theme usage', 'receptar' ); ?></th>
			</tr>
		</thead>
		...
		<tbody>
			...
			foreach ( $image_sizes as $size => $args ) :
				...
				$crop = ( $args['crop'] ) ? ( esc_html__( 'cropped', 'receptar' ) ) : ( esc_html__( 'scaled', 'receptar' ) );

				$row_title = '';
				if ( ! isset( $default_image_size_names[ $size ] ) ) {
					$row_title = __( 'Additional image size added by the theme. Can not be changed on this page.', 'receptar' );
				}
				...
				<tr title="<?php echo esc_attr( trim( $row_title ) ); ?>">
					...
					<td>
						...
						printf(
							...
							esc_html__( '%1$d &times; %2$d, %3$s', 'receptar' ),
							absint( $args['width'] ),
							absint( $args['height'] ),
							esc_html( $crop )
						);
